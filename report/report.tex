...
\section{CriticPilot: GenPilot with Planner-Critic
Mechanism}\label{criticpilot-genpilot-with-planner-critic-mechanism}

\textbf{Course}: Computer Graphics A (Spring 2026 · Fudan University)
--- Project 3
...
GenPilot (Yuan et al., 2025) is a representative work in this field,
...
Inspired by \textbf{PixelCraft\textquotesingle s Planner-Critic
architecture} (Microsoft, 2025)---which introduces a critic agent to
...
\subsubsection{2.1 GenPilot: Test-Time Prompt
Optimization}\label{21-genpilot-test-time-prompt-optimization}

GenPilot (Yuan et al., 2025) is a multi-agent test-time prompt
...
\subsubsection{2.2 PixelCraft: Planner-Critic
Architecture}\label{22-pixelcraft-planner-critic-architecture}

PixelCraft (Microsoft, 2025) is a multi-agent visual reasoning system
...
\pandocbounded{\includegraphics[keepaspectratio,alt={}]{E:/csd3.2/计算机图形学/Project/PlannerCritic-GenPilot/report/overall/CriticPilotArchitecture.png}}

\subsubsection{3.1 Error Aggregator}\label{31-error-aggregator}
...
\subsubsection{3.2 L1 Local Retry}\label{32-l1-local-retry}
...
\subsubsection{3.3 L3 Global Reset}\label{33-l3-global-reset}
...
\subsection{4. Experiments}\label{4-experiments}

\subsubsection{4.1 Experimental Setup}\label{41-experimental-setup}
...
\subsubsection{4.2 Overall Scoring
Performance}\label{42-overall-scoring-performance}
...
essentially flat (Δ = -0.033), all other 9 batches record positive
gains, with Batch4 showing the most significant improvement with Δ
...
\toprule\noalign{}
Batch & Baseline avg & Critic avg & Delta & L1 & L3 & Baseline scored
rounds & Critic scored rounds & Scored rounds difference \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\pandocbounded{\includegraphics[keepaspectratio,alt={}]{E:/csd3.2/计算机图形学/Project/PlannerCritic-GenPilot/report/overall/Baseline-avg-vs-Critic-avg.png}}

Figure 3: CriticPilot Relative to Baseline Image Quality Average Score
Difference

\pandocbounded{\includegraphics[keepaspectratio,alt={}]{E:/csd3.2/计算机图形学/Project/PlannerCritic-GenPilot/report/overall/Delta.png}}

\subsubsection{4.3 Computational Overhead and
Efficiency}\label{43-computational-overhead-and-efficiency}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={}]{E:/csd3.2/计算机图形学/Project/PlannerCritic-GenPilot/report/overall/Baseline-scored-rounds-vs-Critic-scored-rounds.png}}

Figure 5: CriticPilot Relative to Baseline Average Evaluation Rounds
Difference

\pandocbounded{\includegraphics[keepaspectratio,alt={}]{E:/csd3.2/计算机图形学/Project/PlannerCritic-GenPilot/report/overall/Scored-rounds-difference.png}}

\subsubsection{4.4 Mechanism Trigger
Frequency}\label{44-mechanism-trigger-frequency}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={}]{E:/csd3.2/计算机图形学/Project/PlannerCritic-GenPilot/report/overall/L1-and-L3-trigger-counts.png}}

\subsubsection{4.5 Case Analysis}\label{45-case-analysis}
...
and greatly reducing low-score rounds, thus Batch4\textquotesingle s Δ
reached as high as +0.841.

\paragraph{Outlier Analysis: Batch6}\label{outlier-analysis-batch6}

Among all 10 batches, Batch6 is the only one with a negative Δ (-0.033).
...
\item
  ...
  title=\{GenPilot: A Multi-Agent System for Test-Time Prompt
  Optimization in Image Generation\}, \\
  author=\{Wen Ye and Zhaocheng Liu and Yuwei Gui and Tingyu Yuan and
  Yunyue Su and Bowen Fang and Chaoyang Zhao and Qiang Liu and Liang
  Wang\},\\
  year=\{2025\},\\
  eprint=\{2510.07217\},\\
  archivePrefix=\{arXiv\},\\
  primaryClass=\{cs.CV\},\\
  url=\{\url{https://arxiv.org/abs/2510.07217}\}, \\
  \}
\item
  @misc\{zhang2025pixelcraftmultiagenthighfidelityvisual,\\
  title=\{PixelCraft: A Multi-Agent System for High-Fidelity Visual
  Reasoning on Structured Images\}, \\
  author=\{Shuoshuo Zhang and Zijian Li and Yizhen Zhang and Jingjing Fu
  and Lei Song and Jiang Bian and Jun Zhang and Yujiu Yang and Rui
  Wang\},\\
  year=\{2025\},\\
  eprint=\{2509.25185\},\\
  archivePrefix=\{arXiv\},\\
  primaryClass=\{cs.CV\},\\
  url=\{\url{https://arxiv.org/abs/2509.25185}\}, \\
  \}
\item
...
\item
  ...
  plan, reproduced baseline, built basic project code framework, revised
  experimental report
\item
...
\item
  ...
  results
\item
  \textbf{Zhu Yalun(朱雅伦) 24300240187}: Wrote experimental report,
  created PPT
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Appendix B: System Implementation and Reproduction
Guide}\label{appendix-b-system-implementation-and-reproduction-guide}

\subsubsection{Project Structure}\label{project-structure}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{PlannerCritic{-}GenPilot}
...
\NormalTok{├── README.md}
\NormalTok{├── data/}
\NormalTok{│   ├── original\_prompts.txt}
\NormalTok{│   └── original\_prompts\_299.txt}
\NormalTok{├── genpilot/}
\NormalTok{├── pyproject.toml}
\NormalTok{├── run\_baseline.py}
\NormalTok{├── tests/}
\NormalTok{│   ├── test\_compare\_metrics.py}
\NormalTok{│   ├── test\_error\_taxonomy.py}
\NormalTok{│   └── test\_scorer\_json.py}
\NormalTok{└── uv.lock}
\end{Highlighting}
\end{Shaded}

\subsubsection{Installation and Environment
Configuration}\label{installation-and-environment-configuration}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Clone repository}
\FunctionTok{git}\NormalTok{ clone https://github.com/zdleeeeee/PlannerCritic{-}GenPilot.git}
\BuiltInTok{cd}\NormalTok{ PlannerCritic{-}GenPilot}

\CommentTok{\# Copy environment configuration files}
\FunctionTok{cp}\NormalTok{ .env.example .env}
\FunctionTok{cp}\NormalTok{ genpilot/error\_analysis\_pipline.sh.example genpilot/error\_analysis\_pipline.sh}

\CommentTok{\# Install dependencies (using uv)}
\ExtensionTok{uv}\NormalTok{ sync}

\CommentTok{\# Download text{-}to{-}image model (if needed)}
\ExtensionTok{hf}\NormalTok{ download black{-}forest{-}labs/FLUX.1{-}schnell }\AttributeTok{{-}{-}local{-}dir} \OperatorTok{\textless{}}\NormalTok{local download path}\OperatorTok{\textgreater{}}
\end{Highlighting}
\end{Shaded}

\begin{quote}
\textbf{Note}: We upgraded the \texttt{mkl-service} package from 2.4.0
to 2.5.2 in the genpilot environment to resolve its incompatibility with
Python 3.12.
\end{quote}

\subsubsection{Running Process}\label{running-process}

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python}\NormalTok{ run\_baseline.py}
\end{Highlighting}
\end{Shaded}

This script runs both the baseline GenPilot and CriticPilot
simultaneously.
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{tests/test\_error\_taxonomy.py}
\ExtensionTok{tests/test\_scorer\_json.py}
\ExtensionTok{tests/test\_compare\_metrics.py}
\end{Highlighting}
\end{Shaded}
